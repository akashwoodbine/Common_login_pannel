...
import { useAuth } from '../context/AuthContext';
import { DashboardLayout } from './DashboardLayout';
import { DataTableWithDetails } from '@/components/DataTables';
import { BENEFICIARY_DATA } from '../beneficiaryData';
import { Beneficiary, LoginType } from '../types';
...
export const BeneficiaryDetailsPage: React.FC = () => {
  ...
  const filteredBeneficiaries = useMemo(() => {
    return beneficiaries.filter(item => {
      const matchVillage = villageFilter ? item.village === villageFilter : true;
      const matchShg = shgNameFilter ? item.shgName === shgNameFilter : true;
      const matchSocial = socialCategoryFilter ? item.socialCategory === socialCategoryFilter : true;
      const matchStatus = statusFilter ? item.status === statusFilter : true;
      const matchSearch = searchTerm
        ? item.memberName.toLowerCase().includes(searchTerm.toLowerCase()) ||
          item.memberCode.toLowerCase().includes(searchTerm.toLowerCase()) ||
          item.shgCode.toLowerCase().includes(searchTerm.toLowerCase())
        : true;
      return matchVillage && matchShg && matchSocial && matchStatus && matchSearch;
    });
  }, [beneficiaries, searchTerm, villageFilter, shgNameFilter, socialCategoryFilter, statusFilter]);
  ...
  const columns = [
    ...
    { key: 'designationInSHG', label: 'Designation' },
  ...
  const BeneficiaryDetailsModal: React.FC<{ beneficiary: Beneficiary | null; onClose: () => void }> = ({ beneficiary, onClose }) => {
    ...
    const sections = {
      "Personal Info": [
        { label: "Member Name", value: beneficiary.memberName },
        { label: "DOB", value: beneficiary.dateOfBirth },
        { label: "Gender", value: beneficiary.gender },
        { label: "Relation", value: `${beneficiary.fatherMotherSpouseName} (${beneficiary.relation})` },
      ],
      "SHG Info": [
      ...
      <><div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4" onClick={onClose}>
        <div className="bg-white rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
          <div className="p-4 border-b flex justify-between items-center">
            <h2 className="text-xl font-bold">{beneficiary.memberName}</h2>
                ...
                <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-2">
                  {details.map(item => (
                    <div key={item.label}>
                      ...
                      <dd className="text-gray-900">{item.value || 'N/A'}</dd>
                    </div>
                  ))}
                </dl>
              </div>
  ...
  return (
    <><HeaderLangDate /><DashboardLayout title="Beneficiary Details" welcomeContent={welcomeContent}>
      {selectedBeneficiary && <BeneficiaryDetailsModal beneficiary={selectedBeneficiary} onClose={() => setSelectedBeneficiary(null)} />}

      {/* Filters */}
      <div className="bg-white p-6 rounded-lg shadow-md space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <input type="text" placeholder="Search..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full px-3 py-2 border rounded-md" />
          <select value={villageFilter} onChange={e => setVillageFilter(e.target.value)} className="w-full px-3 py-2 border rounded-md">
            <option value="">All Villages</option>
            {uniqueVillages.map(v => <option key={v} value={v}>{v}</option>)}
          </select>
          <select value={shgNameFilter} onChange={e => setShgNameFilter(e.target.value)} className="w-full px-3 py-2 border rounded-md">
            <option value="">All SHG Names</option>
            {uniqueShgNames.map(name => <option key={name} value={name}>{name}</option>)}
          </select>
          <select value={socialCategoryFilter} onChange={e => setSocialCategoryFilter(e.target.value)} className="w-full px-3 py-2 border rounded-md">
            <option value="">All Social Categories</option>
            {uniqueSocialCategories.map(sc => <option key={sc} value={sc}>{sc}</option>)}
          </select>
          ...
          <button onClick={resetFilters} className="w-full px-3 py-2 bg-gray-100 rounded-md">Reset Filters</button>
        </div>
        ...
        <DataTableWithDetails
          ...
          onRowClick={(b: Beneficiary) => setSelectedBeneficiary(b)} />
      </div>
    </DashboardLayout></>
  );
};
